      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 md:gap-8 mb-10">
        ...
        <div>
          <Link to="/" className="flex items-center gap-2 mb-4">
            ...
            <span className="font-heading text-lg font-bold text-background">{name}</span>
          </Link>
          ...
          <div className="space-y-3">
            {branches.map(b => (
              <div key={b.id} className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="text-xs font-semibold text-background/80 mb-0.5">{b.name}</p>
                  {b.address && <p className="text-xs text-background/50 leading-relaxed">{b.address}</p>}
                  {b.map_url && <a href={b.map_url} target="_blank" rel="noreferrer" className="text-[10px] text-primary hover:underline">Get Directions →</a>}
                </div>
              </div>
            ))}
            {phoneDisplay && <a href={`tel:${phone}`} className="flex items-center gap-2.5 text-sm text-background/60 hover:text-primary transition-colors">
