...
export const runSeeder = async (db) => {
  ...
  // ── USERS ──────────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM users WHERE email='admin@mlhk.in'").get())) {
    const hash = bcrypt.hashSync('HarioM9165', 10);
    await db.prepare("INSERT INTO users (id,name,email,password,role,phone) VALUES (?,?,?,?,?,?)").run(uuid(),'Super Admin','admin@mlhk.in',hash,'superadmin','+91 98934 96163');
    console.log('✅ Super Admin created: admin@mlhk.in');
  }
  if (!(await db.prepare("SELECT id FROM users WHERE email='admin@ailaptopwala.com'").get())) {
    const hash = bcrypt.hashSync('Laptop@9165', 10);
    await db.prepare("INSERT INTO users (id,name,email,password,role,phone) VALUES (?,?,?,?,?,?)").run(uuid(),'Admin','admin@ailaptopwala.com',hash,'admin','+91 98934 96163');
  ...
  const upsertSetting = await db.prepare("INSERT OR IGNORE INTO app_settings (key,value) VALUES (?,?)");
  for (const [k,v] of settings) { await upsertSetting.run(k,v); };

  // ── AI AGENT ───────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM ai_agent_settings WHERE id='main'").get())) {
    await db.prepare("INSERT INTO ai_agent_settings (id,enabled,system_prompt) VALUES ('main',0,?)").run(
...
IMPORTANT: Only use products from CURRENT CONTEXT. Never invent prices or specs. Keep responses concise.`
  ...
  // ── CATEGORIES ─────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM categories LIMIT 1").get())) {
    const cats = [
    ...
    const ins = await db.prepare("INSERT OR IGNORE INTO categories (id,name,slug,description) VALUES (?,?,?,?)");
    for (const [n,s,d] of cats) { await ins.run(uuid(),n,s,d); };
    console.log('✅ Categories seeded');
  }

  // ── PRODUCTS ───────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM products LIMIT 1").get())) {
    const ins = await db.prepare(`INSERT OR IGNORE INTO products
    ...
    for (const p of P) { await ins.run(uuid(),...p); };
    console.log('✅ Products seeded');
  }

  // ── SERVICES ───────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM services LIMIT 1").get())) {
    const ins = await db.prepare("INSERT OR IGNORE INTO services (id,name,description,price,duration,category) VALUES (?,?,?,?,?,?)");
    [
    ...
    ].forEach(async ([n,d,p,dur,cat]) => await ins.run(uuid(),n,d,p,dur,cat));
    console.log('✅ Services seeded');
  }

  // ── COUPONS ────────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM coupons LIMIT 1").get())) {
    const ins = await db.prepare("INSERT OR IGNORE INTO coupons (id,code,type,value,min_order,is_active) VALUES (?,?,?,?,?,1)");
    [['ALW10','percentage',10,999],['FLAT200','flat',200,1999],['WELCOME15','percentage',15,999],['FREESHIP','flat',150,0]].forEach(async ([c,t,v,m]) => await ins.run(uuid(),c,t,v,m));
    console.log('✅ Coupons seeded');
  }

  // ── WHATSAPP RULES ─────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM whatsapp_rules LIMIT 1").get())) {
    const ins = await db.prepare("INSERT OR IGNORE INTO whatsapp_rules (id,name,keywords,response_template,type,is_active,match_count) VALUES (?,?,?,?,?,1,0)");
    [
    ...
    ].forEach(async ([n,kw,tmpl,type]) => await ins.run(uuid(),n,JSON.stringify(kw),tmpl,type));
    console.log('✅ WhatsApp rules seeded');
  }

  // ── CMS CONTENT ────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM cms_content LIMIT 1").get())) {
    const ins = await db.prepare("INSERT OR IGNORE INTO cms_content (id,section,content,sort_order,is_active) VALUES (?,?,?,?,1)");
    [
    ...
    ].forEach(async ([s,c,o]) => await ins.run(uuid(),s,JSON.stringify(c),o));
    console.log('✅ CMS content seeded');
  }

  // ── BLOG POSTS ─────────────────────────────────────────
  if (!(await db.prepare("SELECT id FROM blog_posts WHERE status='published' LIMIT 1").get())) {
    const now = new Date().toISOString();
    const ins = await db.prepare('INSERT OR IGNORE INTO blog_posts (id,title,slug,excerpt,content,category,author,image,status,tags,seo_title,seo_description,published_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)');
    [
    ...
    ].forEach(async (p) => await ins.run(uuid(), ...p));
