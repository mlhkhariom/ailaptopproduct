...
router.post('/import', authMiddleware, adminOnly, (req, res) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      const existing = row.sku ? db.prepare('SELECT id FROM products WHERE sku=?').get(row.sku) : (row.id ? db.prepare('SELECT id FROM products WHERE id=?').get(row.id) : null);
      const focusKeywords = row.focus_keywords ? JSON.stringify(row.focus_keywords.split(';').filter(Boolean)) : null;
      if (existing) {
        db.prepare('UPDATE products SET name=?,price=?,original_price=?,category=?,stock=?,in_stock=?,slug=?,badge=?,status=?,description=?,meta_title=?,meta_description=?,focus_keywords=?,image=? WHERE id=?')
...
router.post('/', authMiddleware, adminOnly, (req, res) => {
  ...
  db.prepare(`INSERT INTO products (id,name,name_hi,price,original_price,image,category,description,ingredients,benefits,usage,stock,in_stock,sku,slug,badge,meta_title,meta_description,focus_keywords)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
...
router.put('/:id', authMiddleware, adminOnly, (req, res) => {
  ...
  db.prepare(`UPDATE products SET name=?,name_hi=?,price=?,original_price=?,image=?,category=?,description=?,ingredients=?,benefits=?,usage=?,stock=?,in_stock=?,sku=?,slug=?,badge=?,status=?,meta_title=?,meta_description=?,focus_keywords=? WHERE id=?`)
