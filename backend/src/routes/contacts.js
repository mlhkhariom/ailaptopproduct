...
router.post('/', (req, res) => {
  ...
  const id = uuid();
  db.prepare('INSERT INTO contact_queries (id, name, email, phone, subject, message) VALUES (?, ?, ?, ?, ?, ?)').run(id, name, email, phone, subject, message);
...
router.put('/:id', authMiddleware, adminOnly, (req, res) => {
  ...
  db.prepare('UPDATE contact_queries SET status=?, reply=?, priority=?, starred=? WHERE id=?').run(status, reply, priority, starred ? 1 : 0, req.params.id);
